...
直接看依概率收敛的==**定义**==：设 $\{X_n\}$ 为一随机变量序列，$X$ 为一随机变量，如果对于任意的 $\varepsilon > 0$，有
$$
...
==大数定律的本质==：讨论在什么情况下，随机变量**序列的算术平均 $\frac{1}{n} \sum_{i=1}^n X_i$** 依概率收敛为**均值的算术平均** $\frac{1}{n} \sum_{i=1}^n E (X_i)$。考虑的是一维随机变量（可以将算术平均看作 $Y = g(X_1, X_2, \cdots, X_n)$）向常数的依概率收敛。
...
中心极限定理的本质：讨论在什么情况下，相互独立的随机变量序列和 $Y = \sum_{i=1}^n X_i$ 这个**一维随机变量的分布会收敛于正态分布**。

中心极限定理的定义：如果按照我们之前推算多维随机变量函数分布的做法，对于 $2$ 维、$3$ 维等情况推出 $Y$ 的精确分布可能并不难，但对于 $100$ 维就太复杂了。所以我们就想探究 $n$ 维随机变量序列和的分布与正态分布之间的关系，更具体的是看标准化后的
$$
Y_n^* = \frac{Y_n - E (Y_n)}{\sqrt{Var ( Y_n )}}
$$
在什么条件下才服从正态分布。根据所要求条件的不同，引出中心极限定理的四种形式。
