...
- 每个分量的分布（每个分量的所有信息），即边际分布（3.2）
- 两个分量之间的关联程度，即协方差和相关系数；以及两个分量对整体的表征能力，即对多维变量进行函数替换后的均值（3.3）
- 给定一个分量时，另一个分量的分布，即条件分布（3.4）
...
同样地，直接从边际分布函数的概念出发，再结合概率密度函数的概念，可以得到边际密度函数的概念。如果二维连续随机变量 $(X,Y)$ 的联合密度函数为 $p_{X,Y}(x,y)$ ，因为
$$
\begin{align*}
...
\begin{align*}
  ...
  \end{align*}
$$
对比后发现，**二者分别正好都位于对应变量的密度函数的位置上**，故称这二者为边际密度函数。

##### c. 常见的边际分布

==二维正态分布的边际分布是一维正态分布==，以下求解过程必须牢记在心：
...
$$
改写成（这一步的改写尤为重要，之所以这么改写是因为想要尽可能将 $x$ 和 $y$ 分离开）
$$
...
- 二维正态分布的边际分布中不含参数 $\rho$ （在降维的时候失去了变量间的相关信息）
- 这说明二维正态分布 $N(\mu_1,\mu_2,\sigma_1^2,\sigma_2^2,0.1)$ 与 $N(\mu_1,\mu_2,\sigma_1^2,\sigma_2^2,0.2)$ 的边际分布是相同的。
- ==具有相同边际分布的多维联合分布可以是不同的==，也就是说由联合分布一定能求解出边际分布，但有边际分布却不一定能反解。
...
- 在离散随机变量的场合下直接借助分布列：如果对其任意 $n$ 个取值 $x_1,x_2,\ldots,x_n$ ，有
  $$
  	P(X_1=x_1,X_2=x_2,\ldots,X_n=x_n)=\prod_{i=1}^{n}P(X_i=x_i),
  $$
  则称 $X_1,X_2,\ldots,X_n$ 相互独立。

- 在连续随机变量的场合下直接借助概率密度函数：如果对任意 $n$ 个实数 $x_1,x_2,\ldots,x_n$ ，有
  $$
...
-  **<u>*Step 1*</u>** 由联合概率密度函数/分布列求解出边际概率密度函数/分布列
