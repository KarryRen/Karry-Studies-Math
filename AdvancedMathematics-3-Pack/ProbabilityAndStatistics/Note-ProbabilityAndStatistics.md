...
在上一章中，我们引入统计量的目的是明确的，就是对感兴趣的规律进行准确归纳进而对相关问题进行统计推断（统计量的本质是对随机变量序列的提取，也就是对整体分布某一维度特征的一种具像化表现），详细来说有两层含义：

- 如果已经知道了准确的规律，比如构建 $t$ 统计量时就必须明确知道一个标准正态分布和卡方分布，我们可以基于这些准确的规律构建统计量进行下游任务的判断。
- 如果不知道准确的规律，我们则可以通过以下两种方式进行估计
  - 从样本出发，根据样本构造适当的统计量对总体的规律进行估计
  - 从总体出发，先对规律进行假设，然后将其用到样本中进行验证，验证的过程需要借助统计量
...
这一章我们关注的点是==从现实数据中直接估计出总体分布中的参数，正面给出参数的具体范围，破译分布函数的形式，找到总体的规律==。先明确一下==参数==的广义概念，这里具体指的是以下三类未知参数。注意！！！参数一定是一个数，其有一个确定的值，而绝不是一个随机变量！从这也能看出：参数都是针对总体而言的，总体的参数就是规律的一种表现形式。
...
$$
其中 $a_1,\cdots,a_k$ 是前阶样本原点矩：$a_j=\frac{1}{n}\sum_{i=1}^{n}X_i^j$。从经验上来说当 $k = 1$ 时，也就是说存在 $1$ 个未知参数，我们通常可以由样本均值（一阶原点矩）出发对未知参数进行估计；如果 $k = 2$，也就是说存在 $2$ 个未知参数，可以由样本均值（一阶原点矩阵）和样本方差（二阶中心矩）出发估计未知参数。
...
贝叶斯学派的基本观点是：==任一未知量 $\theta$ 都可看作随机变量，可用一个概率分布去描述，这个分布称为先验分布==。也就是说，“上帝”设定的规律本质上就是一个随机变量，而不想我们之前想的那样是一直是一个一个固定的值。这也可以很自然地理解，可以把“上帝”正向制造规律的过程，想象成它在调节巨大的世界机器，其中每一个零件都是随机变量，有自己的分布函数，由其组合出来的各种规律自然也是随机变量，受每个零件的状态条件而改变。还是那句话世界上没有任何两件事情是绝对独立的，一个规律的产生背后有千千万万个条件，全概率公式极为复杂。所谓先验分布，就是挖掘那些已经被破译的与该规律相关的信息所得到的该规律的分布情况。
...
- 我们的目的是要对总体的未知参数 $\theta$ 作统计推断。在没有样本信息时，我们只能依据先验分布对 $\theta$ 作出推断（假如我们在实验前对整个事件没有任何了解，因此对 $\theta$ 没有任何先验的信息，在这种场合下，贝叶斯本人建议采用“同等无知”的原则==使用区间（0，1）上的均匀分布作为 $\theta$ 的先验分布==，也就是进行**贝叶斯假设**）。在有了样本观察值 $\boldsymbol{X}=\left(x_{1}, \cdots, x_{n}\right)$ 之后，我们应依据 $h(\boldsymbol{X}, \theta)$ 对 $\theta$ 作出推断。若把 $h(\mathbf{X}, \theta)$ 作如下分解:
  ...
  $$
  这个条件分布称为 $\theta$ 的**后验分布**，它集中了总体、样本和先验中有关 $\theta$ 的一切信息。其就是用密度函数表示的贝叶斯公式，也是用总体和样本对先验分布 $\pi(\theta)$ 作调整的结果，它要比 $\pi(\theta)$ 更接近 $\theta$ 的实际情况！（这就是后验分布的含义所在）

==**【贝叶斯估计的典型例子】**==

设某事件 $A$ 在一次试验中发生的概率为 $\theta$，为估计 $\theta$，对试验进行了 $n$ 次独立观测。其中事件 $A$ 发生了 $X$ 次，显然根据**<u>总体信息</u>**我们可以得到：$X | \theta \sim b(n, \theta)$。因为我们之前对事件 $A$ 没有任何的了解，因此没有任何可靠的先验信息，所以只能对 $\theta$ 做出的**<u>先验假设</u>**为：$\theta \sim U(0, 1)$ 。还在实验中观测到事件 $A$ 发生了 $x$ 次这一<u>**样本信息**</u>。我们开始利用贝叶斯公式求出 $\theta$ 在这三重信息下的贝叶斯估计。
...
- **<u>*Step 4*</u>**：求解期望得到最终结果，最后的结果说明 $\theta | x \sim B e(x+1, n-x+1)$，其后验期望估计为
  $$
  \hat{\theta}_{\mathrm{B}}=E(\theta | x)=\frac{x+1}{n+2}
  $$
  与之对比，如果不用先验信息，只用总体信息与样本信息，那么事件 $A$ 发生的概率的最大似然估计为：
  $$
