...
通过前面对概率论的学习，我们发现如果随机变量 $X$ 的分布函数已知，那么其所有和概率相关的规律都可以被推算，也就是说分布函数是对随机变量 $X$ 最本质，最正确的描述。<font color=red>但是！人类真的能够完全准确刻画地随机变量的分布吗（注意不是大概，而是完全精准）？</font> 比如对于扔 $n$ 次硬币朝上的次数 $X$ 这样一个简单的随机变量，我们真的就能够 $100\%$ 把握说它一定服从二项分布 $(n, \frac{1}{2})$，这是不能的！因为我们缺少一个看似”显然“的前提：每扔一次朝上的概率是 $\frac{1}{2}$。而我们可以肯定，这个”显然“的前提并不一定 $100\%$ 出现，原因很简单，可能硬币制作的时候出了瑕疵，可能硬币本来就是三个面的。

因此，在现实中我们思考问题的方式应该是完全反过来的。在承认以下两个前提的基础上才能展开思考：

- 随机变量 $X$ 的确服从一个确定的分布（其可能随着时间或者其他条件变化，但是当所有控制条件都给定时，它的形式是确定）
- 我们无法从根源上准确地得到这个分布的形式，只能从事实出发进行估计

回到扔硬币的例子，问题的描述方式就完全改变了，对于扔硬币正面朝上 $X$ 这样一个简单的随机变量，“上帝”已然设定好每次实验正面朝上的概率是 $p$，理性上讲：世界上没有一个概率是独立的，都是条件概率的情况下，必须要通过全概率公式求解，但是谁又能把世间所有相关的事件都找出来呢？（这个概率是世界运作的规律，==是确定的，不随着人的意志而转移==，也就是说不管你实验多少次，如何实验，每次硬币正面朝上的概率都是 $p$）。这样“上帝”也就间接设定 $X$ 服从二值分布 $b(n,p)$（这就是确定的分布）。因此如果我们确定了 $p$，所有的附加规律就都知晓了，就可以借助它帮助我们研究更加具体的结论。但是悲哀的事情是，我们永远也没办法从根源上准确破译它：
...
这正是统计学研究的问题，和微积分那种发现确定规律的理论不同，统计旨在通过收集和分析现实生活中受到随机影响的数据（比如抛 $1 \sim 100$ 次硬币正面朝上的次数集合），以概率的思想，==以极大概率的结论逼近确定的规律。==因此统计的本质就是基于可观测到的数据挖掘世界的本质规律。因为概率的存在，所以观测到的数据肯定存在各种各样的白噪声，如何透过现象看本质，是统计学始终在问的问题。
...
>
> 但是永远不能做出这样的判断那就是我有 $100\%$ 的把握认为 $p \le 0.05$ ，因为我们无法从根源上准确地破译它，只能基于能够拿到的数据来做，这显然始终是受到限制的！
...
既然样本是一个独立同分布的随机变量序列，那我们就完全可以用之前所学的大数定律、中心极限定理等工具对其进行分析，更统一的工具是**使用多维随机变量的函数实现变化特征降维，产生新的一维随机变量**。所谓统计量就是采用一系列不同的、具有特殊含义的函数对样本实现降维提取特征后得到的对应的、有一定含义的一维随机变量【统计量仍然是随机变量】。
...
$$
在实际中，$s^2$ 比 $s_n^2$ 更常用，在以后讲样本方差通常指的是 $s^2$。
...
- 均值和方差相互独立： $\overline X$ 与 $s^2$ 相互独立 （难以证明，记住即可）

- 均值服从正态分布 $\overline X \sim N(\mu,\sigma^2/n)$ （卷积公式，结果比较显然）

- 方差标准化后服从卡方分布 （将方差化开，配到卡方分布的定义式上完成证明）
  $$
...
在刚接触到样本的统计量时我们就说，统计量是帮组我们理解总体的工具。更加准确地来说，构造统计量就是对样本进行加工，去粗取精，简化样本，便于统计推断发现总体规律。
...
$$
就像是完成了因子分解，把与 $\theta$ 有关的统计特征分解到一个函数中了。
